...
if __name__ == '__main__':
    ...
    # 测试实际存在的库合约
    # res = combine_by_index([2], read_compiled_contract())
    # contractAddress = deploy(*res)
    # contract = web3.eth.contract(address=contractAddress, abi=res[0])
    # print(contract.functions.encode('0x33').call())
    ...
    # 3
    lib = read_compiled_contract()
    t_bytecode = addition_to_source(Addition(lib[2][2])).bytecode
    contractAddress = deploy(lib[2][1], t_bytecode)
    contract = web3.eth.contract(address=contractAddress, abi=lib[2][1])
    print(contract.functions.encode('0x33').call())
